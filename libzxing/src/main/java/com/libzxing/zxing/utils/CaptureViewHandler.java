...
package com.libzxing.zxing.utils;
...
public class CaptureViewHandler extends Handler {
    ...
    public CaptureViewHandler(ScannerView scannerView, CameraManager cameraManager, int decodeMode) {
        this.mScannerView = scannerView;
        decodeThread = new DecodeThread(mScannerView, decodeMode);
        decodeThread.start();
        state = State.SUCCESS;
    ...
    public void handleMessage(Message message) {
        if (message.what == R.id.restart_preview) {
            ...
            state = State.SUCCESS;
            if(message.getData() == null) { //防止出错
                state = State.PREVIEW;
                cameraManager.requestPreviewFrame(decodeThread.getHandler(), R.id.decode);
            }else{
                mScannerView.getResultHandler().feedback(message.getData());
            }
            ...
            Activity activity = (Activity) mScannerView.getContext();
